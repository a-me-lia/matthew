...
export async function generateMetadata({
  ...
  const post = allMeta.find((post) => post.slug === params.slug);
  if (!post) {
    return;
  }
  ...
  return {
    ...
    openGraph: {
      ...
      type: "article",
      publishedTime,
      url: `https://matthewguo.com/meta/${slug}`,
      images: [
        {
          url: ogImage,
        },
...
export default async function Blog({ params }: { params: any }) {
  ...
  return (
    <section className="min-h-screen bg-white mx-4  md:mx-auto md:w-[742px] mb-64  relative">
      ...
      <h1 className="font-bold text-2xl max-w-[600px]  mt-16 md:mt-32">
      ...
      <div className="flex justify-between space-y-2 md:space-y-0 md:items-center mt-2 mb-8 text-sm max-w-[650px] flex-col md:flex-row">
        ...
        <div className="flex flex-row">
          <Tag url="meta" tags={post.tags}></Tag>
        </div>
      </div>
